...
class Korisnik {
    ...
    public static function registracija($username,$password,$email,$tip_korisnika_id) {
        ...
        $database->insert('Korisnik','INSERT INTO korisnici (username, password, email, tip_korisnika_id) VALUES (:username, :password, :email, :tip_korisnika_id);',
        [
            ':username' => $username,
            ':password' => $password,
            ':email' => $email,
    ...
    public static function proveri($username,$password) {
        $password = hash('sha512',$password);
        $database = Database::getInstance();
        $korisnici = $database->select('Korisnik', 'SELECT * FROM korisnici WHERE username LIKE :username AND password LIKE :password',
        [
            ':username' => $username,
            ':password' => $password
        ]);
    ...
    public static function korisnik_za_id($id) {
        ...
        $database = Database::getInstance();
        $korisnici = $database->select('Korisnik', 'SELECT * FROM korisnici WHERE id=:id',
        [
            ':id' => $id
        ]);
